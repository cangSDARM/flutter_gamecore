...
class Sets extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: Appbar(titleNormal: AppTextTitle(this.title, i18n: false)),
      body: ListView(children: listBuilder(context)),
  ...
  List<Widget> listBuilder(BuildContext context) {
    return this
        ...
        .map((e) => InkWell(
            child: Container(
                decoration: BoxDecoration(border: setsStyle["containerBorder"]),
                width: double.infinity,
                alignment: Alignment.centerLeft,
                padding: setsStyle["padding"],
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: <Widget>[Text(e.title, style: setsStyle["titleStyle"]), setsStyle["icon"]],
                )),
            onTap: () {
              if (e.subsets != null) {
  ...
  MaterialPageRoute subSets(dynamic e) {
    ...
    return MaterialPageRoute(builder: (_) => Sets(sets: e.subsets, title: e.title));
  }
}
